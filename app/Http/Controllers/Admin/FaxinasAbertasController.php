...
class FaxinasAbertasController extends Controller
{
    public function index()
    {
        if (! Gate::allows('faxinas_aberta_access')) {
            return abort(401);
        }
		...
		if (request()->ajax()) {
			...
			$status_id = \App\CleaningStatus::where('title', 'Aberta')->first()->id;
			$query->where('status_id', $status_id);
			$query->whereDoesntHave('clean_slots',
				function ($query) {
					$query->where('user_id', \Auth::user()->id);
			});
			//$query->whereHas("clean_slots");
			$query->has("free_slots");
			$city = \Auth::user()->city;
			if($city){
				$query->whereHas('client',
					...
					})->with(['client', 'clean_slots', 'clean_category', 'clean_type']);
			}
        ...
        return view('admin.faxinas_abertas.index');
    }
}
